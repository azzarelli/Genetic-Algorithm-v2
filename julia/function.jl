...
function child_str(a::Int, b::Int)
    if a >= b
        return a
    else
        return b
...
function child_int(a::Int, b::Int, rand::Int)
    if ((a && b) >= 6) && (rand > 1) # rand is 0 to 5 so 1/5 chance super-intelligence isn't passed on 
    ...
    elseif ((a || b) >= 6) && (rand > 1) # either parents smart still a chance the kid is too
        if a > 4 && b > 4 && (rand > 2) # now 3/5 chance of passing on intelligence as parents get less intelligen
            return 6
        else 
            return 5
        end
    elseif (a || b) > 3 && rand > 3 #now 2/5 chance of intelligence being passed on
        return 4
    else
        return convert(Int, (a + b)/2)
    end
end

function child_beau(a::Int, b::Int, rand::Int)
